...
namespace TooJpeg
{
  // Optional one-shot encode profiler (perf "Lever #2" step 0). When the encoder
  // is built with -DTOOJPEG_PROFILE, writeJpeg accumulates per-phase CPU timebase
  // ticks into a caller-supplied struct, so we can see where encode time actually
  // goes: per-pixel RGB->YCbCr conversion vs forward DCT+quantize vs Huffman
  // entropy coding. The struct is supplied (and owned) by the caller per call -
  // there are no globals, so it stays correct across the parallel strip workers.
  // Pass nullptr (the default) to skip profiling. Convert ticks to microseconds
  // on the caller side (OSTicksToMicroseconds).
  struct EncodeProfile {
    unsigned long long colorTicks;    // RGB->YCbCr (incl. 4:2:0 chroma averaging)
    unsigned long long dctQuantTicks; // forward DCT + AAN scale + quantize/zigzag
    unsigned long long entropyTicks;  // run-length + Huffman bit writing
  };
  ...
  // profile     - optional out-param; if non-null AND built with -DTOOJPEG_PROFILE, receives a per-phase tick breakdown (added to its current value)
